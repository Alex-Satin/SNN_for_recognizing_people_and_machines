...
# Шляхи до результатів обох моделей
model_paths = {
    'YOLOv8n-1COCO': 'runs/detect/yolov8n_custom/results.csv',
    'YOLOv8n-1balanced_dataset': 'runs/detect/yolov8n_trein_coco_balanced_dataset/results.csv'
}


# Загальна папка для збереження
base_save_dir = os.path.join(os.path.dirname(__file__), 'yolo_results')
...
for model_name, csv_path in model_paths.items():
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(model_save_dir, f'{model_name}_losses.png'))
    plt.close()
    ...
    # --- Таблиця метрик з останньої епохи ---
    last = df.iloc[-1]
    metrics_table = {
    ...
    txt_path = os.path.join(model_save_dir, f"{model_name}_metrics.txt")
    ...
    # --- Матриця метрик по епохах ---
    matrix_csv_path = os.path.join(model_save_dir, f"{model_name}_metrics_per_epoch.csv")
    df_metrics = df[[
